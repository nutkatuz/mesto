...
// Данная функция (setEventListeners) должна вызываться только один раз из index.js._removeEventListeners - оставить в ней только удаление обработчика на ESC, т.к. остальные будут внутри функции setEventListeners, которая будет вызвана только один раз, и эти обработчики не нужно будет удалять. Добавление обработчиков(на крестик и оверлей) для каждого модального окно должно происходить только один раз, а не при каждом открытии. Пожалуйста, вынесите вызов этой функции в глобальный скоуп. Также не забудьте это исправить также с другими модальными окнами